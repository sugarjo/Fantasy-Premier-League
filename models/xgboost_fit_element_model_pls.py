...
def objective_xgboost(space):
    ...
    HO_X = val_X.loc[val_selected, columns_to_keep]  
    ...
    hold_out_X = HO_X.copy()
    hold_out_y = val_y.copy()[val_selected]
    ...
    for k in fit_X.keys():
        remove = False
        for c in ['string_team', 'season', 'string_opp_team', 'name']:
            if c in k:
                remove = True
        ...
        #do not impute those that miss many values
        if sum(fit_X[k].isna())/fit_X.shape[0] > 0.2:
            nan_cols.append(k)          
        elif not remove:
            num_cols.append(k)
        else:
        ...
        # #convert to floats
        # if 'difficulty' in k:
        #     eval_X[k] = eval_X[k].astype(float)
        #     fit_X[k] = fit_X[k].astype(float)
        #     hold_out_X[k] = hold_out_X[k].astype(float)
          
        # #log transform
        # for c in ['transfers_in', 'transfers_out', 'minutes', 'ict_index', 'defcon']:
        #     if c in k:
        #         eval_X[k] = np.log1p(eval_X[k]+1)
        #         fit_X[k] = np.log1p(fit_X[k]+1)
        
    y_train = fit_y.values.reshape(-1, 1)
    y_test  = eval_y.values.reshape(-1, 1)
    y_HO  = hold_out_y.values.reshape(-1, 1)
        
    scaler_y = StandardScaler()
    y_train_scaled = scaler_y.fit_transform(y_train)
    y_test_scaled  = scaler_y.transform(y_test)
    y_HO_scaled  = scaler_y.transform(y_HO)
    
    if num_cols:
        imputer = SimpleImputer(strategy="median")  # or "median" for skewed data
        X_train_imp = imputer.fit_transform(fit_X[num_cols])
        X_test_imp  = imputer.transform(eval_X[num_cols])
        X_HO_imp  = imputer.transform(hold_out_X[num_cols])
        
        scaler_X = StandardScaler()
        X_train_scaled = scaler_X.fit_transform(X_train_imp)
        X_test_scaled  = scaler_X.transform(X_test_imp)
        X_HO_scaled  = scaler_X.transform(X_HO_imp)
        
        if 'num_PLS' in space.keys():    
            num_pls = int(space['num_PLS'])
            
            if num_pls > np.min(X_train_scaled.shape):
                num_pls = np.min(X_train_scaled.shape)
        ...
        pls_final = PLSRegression(n_components=num_pls)
        pls_final.fit(X_train_scaled, y_train_scaled)
        
        T_fit = pls_final.x_scores_   
        T_eval = pls_final.transform(X_test_scaled)
        T_HO = pls_final.transform(X_HO_scaled)
        
        T_train_df = pd.DataFrame(
            T_fit,
            index=fit_X.index,
            columns=[f"PLS_{i+1}" for i in range(num_pls)]
            )
        
        T_eval_df = pd.DataFrame(
                T_eval,
                index=eval_X.index,
                columns=[f"PLS_{i+1}" for i in range(num_pls)]
            )
        
        T_HO_df = pd.DataFrame(
                T_HO,
                index=HO_X.index,
                columns=[f"PLS_{i+1}" for i in range(num_pls)]
            )
    ...
    T_train_df[cat_cols] = fit_X[cat_cols]
    T_eval_df[cat_cols] = eval_X[cat_cols]   
    T_HO_df[cat_cols] = hold_out_X[cat_cols]   
    
    T_train_df[nan_cols] = fit_X[nan_cols]
    T_eval_df[nan_cols] = eval_X[nan_cols]   
    T_HO_df[nan_cols] = hold_out_X[nan_cols] 
    
    dfit = xgb.DMatrix(data=T_train_df, label=y_train_scaled, enable_categorical=True)
    deval = xgb.DMatrix(data=T_eval_df, label=y_test_scaled, enable_categorical=True)
    dHO = xgb.DMatrix(data=T_HO_df, label=y_HO_scaled, enable_categorical=True)
    ...
    if not model.get_score():
        ...
        val_trans_pred = model.predict(dHO)
        val_trans_pred  = val_trans_pred.reshape(-1, 1)
        
        val_pred = scaler_y.inverse_transform(val_trans_pred)
    
        
        val_error = np.mean(np.abs(np.array(val_y[val_selected]) - val_pred)**2)
...
def plot_objective_xgboost(space):
    ...
    #make sure all categories in val_x is present in cv_x
    for column in hold_out_X.columns:
        if isinstance(hold_out_X[column].dtype, pd.CategoricalDtype):
            # Get the values in the current column of val_X
            val_values = hold_out_X[column]
            
            # Check which values are present in the corresponding column of cv_X
            mask = val_values.isin(fit_X[column])
            
            # Set values that are not present in cv_X[column] to NaN
            hold_out_X.loc[~mask, column] = np.nan
    ...
    if not model.get_score():
        ...
        num_features = T_train_df.shape[1]
...
for pos in range(1,5):
    ...
    hyperparam_path = main_directory + f'\models\hyperparams{pos}_pls.pkl'
    ...
    space={'max_depth': hp.qloguniform("max_depth", 1, np.log(70), 1), 
            'min_split_loss': hp.loguniform('min_split_loss', 0, np.log(150)), #log?
            'reg_lambda' : hp.uniform('reg_lambda', 0, 300),
            'reg_alpha': hp.uniform('reg_alpha', 0.01, 500),
            'min_child_weight' : hp.uniform('min_child_weight', 0, 250),
            'learning_rate': hp.uniform('learning_rate', 0, 4),
            'subsample': hp.uniform('subsample', 0.1, 1),
            'colsample_bytree': hp.uniform('colsample_bytree', 0.1, 1),
            'colsample_bylevel': hp.uniform('colsample_bylevel', 0.1, 1),
            'colsample_bynode': hp.uniform('colsample_bynode', 0.1, 1),
            'early_stopping_rounds': hp.quniform("early_stopping_rounds", 1, 2000, 1),
            'eval_fraction': hp.uniform('eval_fraction', min_eval_fraction, 0.1),
            'n_estimators': hp.quniform('n_estimators', 2, 15000, 1),
            'max_delta_step': hp.uniform('max_delta_step', 0, 120),
            'grow_policy': hp.choice('grow_policy', [0, 1]), #1
            'max_leaves': hp.quniform('max_leaves', 0, 75, 1),
            'max_bin':  hp.qloguniform('max_bin', np.log(2), np.log(50), 1),
            'temporal_window': hp.quniform('temporal_window', 1, 20, 1),
            'num_PLS': hp.quniform('num_PLS', 1, 25, 1),
        }
