...
def objective_xgboost(space):
    ...
    if not model.get_score():
        ...
        val_error = np.mean(np.abs(np.array(val_y) - val_pred)**2)
...
for ind, name in enumerate(unique_names):
    ...
    space={'max_depth': hp.qloguniform("max_depth", 1, np.log(100), 1), 
            ...
            'n_estimators': hp.quniform('n_estimators', 2, 3500, 1),
            'max_delta_step': hp.uniform('max_delta_step', 0, 100),
            'grow_policy': hp.choice('grow_policy', [0, 1]), #1
            'max_leaves': hp.quniform('max_leaves', 0, 80, 1),
            'max_bin':  hp.qloguniform('max_bin', np.log(2), np.log(130), 1),
            'temporal_window': hp.quniform('temporal_window', 1, 17, 1),
            'num_PLS': hp.quniform('num_PLS', 1, 20, 1),
        }
    ...
    for X1, y1, X2, y2, mask1, mask2 in zip((cv_X, val_X), (cv_y, val_y), (val_X, cv_X), (val_y, cv_y), (cvs_mask, vals_mask), (vals_mask, cvs_mask)):
        ...
        # Get the 80% of the first matches every season...
        objective_X = X1.reset_index(drop=True)
        HO_X = X2.copy()
        ...
        if not model.get_score():
            ...
            val_error = np.mean(np.abs(np.array(y2) - val_pred)**2)
            ...
            train_data.loc[rows_to_update, 'local_pls_predictions'] = val_pred
...
train_data.to_pickle(r'\\platon.uio.no\med-imb-u1\jorgels\\model_local_pls_data.pkl')  # Set index=False to not include row indices