...
def custom_metric(pred_y, dtrain):
    ...
    if dtrain.num_row() == fit_X.shape[0]:
        mask = fit_X.running_minutes > 60
    elif dtrain.num_row() == eval_X.shape[0]:
        mask = eval_X.running_minutes > 60
...
def objective_xgboost(space):
    ...
    val_error = mean_squared_error(val_y[selected], val_pred[selected])
...
if method == 'linear_reg':
    ...
    dval = xgb.DMatrix(data=val_X, label=val_y, enable_categorical=True)
